...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(int hour, int minutes, int remind, String repeat, String date) {
    ...
    final tz.TZDateTime formattedDate = tz.TZDateTime.from(DateTime.parse(date), tz.local);
    ...
    if (scheduledDate.isBefore(now)) {
      if (repeat == 'Daily') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month, DateTime.parse(date).day + 1, hour, minutes);
      } else if (repeat == 'Weekly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month, DateTime.parse(date).day + 7, hour, minutes);
      } else {
        scheduledDate =
            tz.TZDateTime(tz.local, now.year, DateTime.parse(date).month + 1, DateTime.parse(date).day, hour, minutes);
      }
      scheduledDate = reminder(remind, scheduledDate);
    }
